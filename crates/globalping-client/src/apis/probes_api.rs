...
/// struct for typed errors of method [`list_probes`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListProbesError {
...
pub async fn list_probes(configuration: &configuration::Configuration, ) -> Result<Vec<models::Probe>, Error<ListProbesError>> {

    let uri_str = format!("{}/v1/probes", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::Probe&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::Probe&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListProbesError> = serde_json::from_str(&content).ok();
