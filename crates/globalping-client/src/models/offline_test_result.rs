...
pub struct OfflineTestResult {
    #[serde(rename = "status")]
    pub status: Box<models::OfflineTestStatus>,
...
impl OfflineTestResult {
    /// Represents an `offline` test where the requested probe was not available to run the test and most fields are not available. Only possible when you pass the `id` of a previous measurement in the `locations` field. 
    pub fn new(status: models::OfflineTestStatus, raw_output: String) -> OfflineTestResult {
        OfflineTestResult {
